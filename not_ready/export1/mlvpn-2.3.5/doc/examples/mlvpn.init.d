...
#              in /mlvpn/mlvpn/etc/default/mlvpn and /mlvpn/mlvpn/etc/mlvpn/*.conf
### END INIT INFO
...
DAEMON=/mlvpn/mlvpn/sbin/mlvpn
RUNDIR=/mlvpn/mlvpn/var/run/mlvpn
USER=mlvpn
DESC="virtual private network daemon"
CONFIG_DIR=/mlvpn/mlvpn/etc/mlvpn
...
AUTOSTART="all"
if test -e /mlvpn/mlvpn/etc/default/mlvpn ; then
  . /mlvpn/mlvpn/etc/default/mlvpn
fi
...
start_vpn () {
    ...
    start-stop-daemon --start --quiet --oknodo \
        --pidfile /mlvpn/mlvpn/var/run/mlvpn.$NAME.pid \
        --background \
        --make-pidfile \
...
stop)
  ...
  if test -z "$2" ; then
    for PIDFILE in `ls /mlvpn/mlvpn/var/run/mlvpn.*.pid 2> /dev/null`; do
    ...
    while shift ; do
      [ -z "$1" ] && break
      if test -e /mlvpn/mlvpn/var/run/mlvpn.$1.pid ; then
        PIDFILE=`ls /mlvpn/mlvpn/var/run/mlvpn.$1.pid 2> /dev/null`
        NAME=$(basename $PIDFILE | cut -f2- -d'.')
        NAME=${NAME%%.pid}
        stop_vpn
        log_progress_msg "$NAME"
  ...
  if test -z "$2" ; then
    ...
    for CONFIG in `cd $CONFIG_DIR; ls *.conf 2> /dev/null`; do
      ...
      if test "x$AUTOVPN" = "x1" ; then
        ...
        status_of_proc -p /mlvpn/mlvpn/var/run/mlvpn.${NAME}.pid mlvpn "VPN '${NAME}'" || GLOBAL_STATUS=1
      else
        status_of_proc -p /mlvpn/mlvpn/var/run/mlvpn.${NAME}.pid mlvpn "VPN '${NAME}' (non autostarted)" || true
      fi
    done
  else
